...
class SettingsFragment : Fragment() {
    ...
    private fun driveKeyToastFunc(context: Context, position:Int){
        ...
        {
            Toast.makeText(context, "Режимы вождения по кнопке доступны только при включенном 'Восстановлении режима вождения при запуске'", Toast.LENGTH_LONG ).show()
            driveKeyToast = false
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        importSett.setOnClickListener(View.OnClickListener() {
            ...
            if(!ret){
                Toast.makeText(context, "Ошибка импорта настроек", Toast.LENGTH_LONG).show()
            }else{
                Toast.makeText(context, "Настройки импортированы", Toast.LENGTH_LONG).show()
            }
        })
